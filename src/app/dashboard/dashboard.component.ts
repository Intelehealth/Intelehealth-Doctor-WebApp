...
import {
  ...
  CustomObsModel,
  CustomVisitModel,
  PatientVisitSummaryConfigModel,
  ProviderAttributeModel,
  RescheduleAppointmentModalResponseModel,
} from "../model/model";
...
export class DashboardComponent implements OnInit {
  ...
  @ViewChild(CompletedVisitsComponent)
  completedVisitsComponent: CompletedVisitsComponent;
  @ViewChild(FollowupVisitsComponent)
  followUpVisitsComponent: FollowupVisitsComponent;
  ...
  constructor(
    ...
    this.displayedColumns2 = this.displayedColumns2.filter(
      (col) => col !== "age" || this.checkPatientRegField("Age")
    );
    this.displayedColumns3 = this.displayedColumns3.filter(
      (col) => col !== "age" || this.checkPatientRegField("Age")
    );
    this.displayedColumns4 = this.displayedColumns4.filter(
      (col) => col !== "age" || this.checkPatientRegField("Age")
    );

    if (!this.pvs.awaiting_visits_patient_type_demarcation) {
      this.displayedColumns3 = this.displayedColumns3.filter(
        (col) => col !== "patient_type"
      );
    }
  }
  ...
  ngOnInit(): void {
    ...
    if (provider) {
      ...
      if (this.pvs?.completed_visit_section) this.getCompletedVisits();

      if (this.pvs?.follow_up_visit_section) this.getFollowUpVisit();
    }
  ...
  initHelpTour() {
    if (
      getCacheData(false, doctorDetails.IS_NEW_DOCTOR) ===
        ...
        if (res.success) {
          ...
          for (let i = 0; i < res.data.length; i++) {
            ...
            if (visit?.encounters?.length) {
              ...
              visit.completed = this.getEncounterCreated(
                visit,
                visitTypes.VISIT_COMPLETE
              );
              visit.followUp = this.getEncounterObs(
                visit.encounters,
          ...
          if (page == 1) {
            ...
            this.dataSource6.filterPredicate = (
              data: {
              ...
              data?.patient_name.given_name
                .concat(
                  ...
                  this.checkPatientRegField("Middle Name")
                    ? " " + data?.patient_name.middle_name
                    : "") +
                    " " +
  ...
  getCompletedVisits(page: number = 1) {
    this.visitService
      .getEndedVisits(this.specialization, page)
      .subscribe((res: ApiResponseModel) => {
        if (res.success) {
          ...
          for (let i = 0; i < res.data.length; i++) {
            ...
            visit.TMH_patient_id = this.getAttributeData(
              visit,
              "TMH Case Number"
            );
          ...
          if (page == 1) {
            ...
            this.dataSource5.filterPredicate = (
              data: {
                patient: { identifier: string };
                patient_name: {
                  given_name: string;
  ...
  getAwaitingVisits(page: number = 1) {
    ...
    this.visitService
      .getAwaitingVisits(this.specialization, page)
      .subscribe((av: ApiResponseModel) => {
        if (av.success) {
  ...
  getDemarcation(enc) {
    ...
    const adlIntl = enc?.find?.(
      (e) => e?.type?.name === visitTypes.ADULTINITIAL
    );
    if (Array.isArray(adlIntl?.obs)) {
      adlIntl?.obs.forEach((obs) => {
        if (!isFollowUp)
          isFollowUp = obs?.value_text
            ?.toLowerCase?.()
            ?.includes?.("follow up");
      });
    }
        ...
        appointmentsdata.forEach((appointment: AppointmentModel) => {
          if (
            ...
            if (appointment.visit) {
              ...
              appointment.telephone = this.getTelephoneNumber(
                appointment?.visit?.person
              );
              appointment.TMH_patient_id = this.getAttributeData(
                appointment.visit,
                "TMH Case Number"
              );
    ...
    encounters.forEach((encounter: CustomEncounterModel) => {
      ...
      if (display.match(encounterName) !== null) {
        created_at = this.getCreatedAt(
          encounter.encounter_datetime.replace("Z", "+0530")
        );
      }
    });
    return created_at;
  ...
  getEncounterObs(
    ...
    encounters.forEach((encounter: CustomEncounterModel) => {
      if (encounter.type?.name === encounterName) {
        obs = encounter?.obs?.find(
          (o: CustomObsModel) => o.concept_id == conceptId
        );
      }
    });
    return obs;
                ...
                if (result) {
                  ...
                  this.appointmentService
                    ...
                    .subscribe((res: ApiResponseModel) => {
                      ...
                      if (res.status) {
                        this.mindmapService.notifyHwForRescheduleAppointment(
                          appointment
                        );
                        this.getAppointments();
                        this.toastr.success(
  ...
  getTelephoneNumber(person: AppointmentModel["visit"]["person"]): any {
    return person?.person_attribute.find(
      (v: { person_attribute_type_id: number }) =>
        v.person_attribute_type_id == 8
    )?.value;
  }
  ...
  convertToDate(relativeTime: string): string {
    ...
    if (["hour", "hours"].includes(unit.toLowerCase()))
      ...
      throw new Error(
        'Invalid time unit. Only "hours", "minutes", or "days" are supported.'
      );
  ...
  applyDateOrRangeFilter2() {
    ...
    if (selectedDate) {
      ...
      this.dataSource2.filterPredicate = (data: any, filter: string) => {
        const itemDate = data.visit_created.includes(",")
          ? this.formatDate(data.visit_created)
          : this.convertToDate(data.visit_created);
        return itemDate === filter;
      };
  ...
  applyDateOrRangeFilter4() {
    ...
    if (selectedDate) {
      ...
      this.dataSource4.filterPredicate = (data: any, filter: string) => {
        const itemDate = data.prescription_started.includes(",")
          ? this.formatDate(data.prescription_started)
          : this.convertToDate(data.prescription_started);
        return itemDate === filter;
      };
