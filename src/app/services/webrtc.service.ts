...
export class WebrtcService {
      ...
      .on(RoomEvent.Connected, async () => {
        try {
          ...
          if (error.name === 'NotAllowedError') {
            console.error('Camera or microphone access is denied. Please check your browser permissions.');
          } else if (error.name === 'NotFoundError') {
            console.error('No media devices found. Please make sure your camera and microphone are connected.');
          } else {
            console.error('An unknown error occurred while accessing the camera and microphone.');
          }
          // location.reload()
        }
      })
  ...
  handleTrackSubscribed(
    ...
    if (track.kind === Track.Kind.Audio) {
      ...
      element.style['object-fit'] = 'contain'
      this.remoteContainer.appendChild(element);
    }
  }
