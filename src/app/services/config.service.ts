...
export class ConfigService {
   ...
   * @return {Observable<any>}
   */
  getRosterQuestionnaire(): Observable<any> {
    const url = `${this.baseURL}/roster-questionnaire/all`;
    return this.http.get(url);
  }
  /**
   * Update feature enabled status
   ...
   * @return {Observable<any>}
   */
  updateRosterQuestionnaireStatus(
    id: number,
    is_enabled: boolean
  ): Observable<any> {
    const url = `${this.baseURL}/roster-questionnaire/updateIsEnabled/${id}`;
    return this.http.put(url, { is_enabled });
  }
