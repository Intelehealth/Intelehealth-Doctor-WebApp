...
export class LibPresciptionComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private libPresciptionService: LibPresciptionService,
  ...
  */
  getVisit(uuid: string) {
    this.libPresciptionService.fetchVisitDetails(uuid).subscribe((visit: VisitModel) => {
      if (visit) {
        this.visit = visit;
        this.checkVisitStatus(visit.encounters);
        this.libPresciptionService.patientInfo(visit.patient.uuid).subscribe((patient: PatientModel) => {
          if (patient) {
      ...
      if (enc.encounterType.display === visitTypes.ADULTINITIAL) {
        enc.obs.forEach((obs: ObsModel) => {
          if (obs.concept.display === visitTypes.CURRENT_COMPLAINT) {
            const currentComplaint =  this.libPresciptionService.getData(obs)?.value.replace(new RegExp('►', 'g'), '').split('<b>');
  ...
  */
  checkIfDiagnosisPresent() {
    ...
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptDiagnosis).subscribe((response: ObsApiResponseModel) => {
  ...
  */
  checkIfNotePresent() {
    this.notes = [];
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptNote).subscribe((response: ObsApiResponseModel) => {
      response.results.forEach((obs: ObsModel) => {
        if (obs.encounter.visit.uuid === this.visit.uuid) {
          this.notes.push(obs);
        }
  ...
  */
  checkIfMedicationPresent() {
    this.medicines = [];
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptMed).subscribe((response: ObsApiResponseModel) => {
      response.results.forEach((obs: ObsModel) => {
        if (obs.encounter.visit.uuid === this.visit.uuid) {
  ...
  */
  checkIfAdvicePresent() {
    this.advices = [];
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptAdvice)
  ...
  */
  checkIfTestPresent() {
    this.tests = [];
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptTest)
      .subscribe((response: ObsApiResponseModel) => {
        response.results.forEach((obs: ObsModel) => {
          if (obs.encounter && obs.encounter.visit.uuid === this.visit.uuid) {
            this.tests.push(obs);
  ...
  */
  checkIfReferralPresent() {
    this.referrals = [];
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptReferral)
      .subscribe((response: ObsApiResponseModel) => {
        response.results.forEach((obs: ObsModel) => {
  ...
  */
  checkIfFollowUpPresent() {
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptFollow).subscribe((response: ObsApiResponseModel) => {
      response.results.forEach((obs: ObsModel) => {
        if (obs.encounter.visit.uuid === this.visit.uuid) {
  ...
  */
  detectMimeType(b64: string): string {
    return this.libPresciptionService.detectMimeType(b64);
  }
  ...
  */
  ...
  checkIfFollowUpInstructionsPresent(): void {
    ...
    this.libPresciptionService.getObs(this.visit.patient.uuid, this.conceptFollowUpInstruction).subscribe((response: ObsApiResponseModel) => {
      response.results.forEach((obs: ObsModel) => {
        if (obs.encounter.visit.uuid === this.visit.uuid) {
